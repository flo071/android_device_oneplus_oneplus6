...
if ! grep android.hardware.biometrics.fingerprint /vendor/manifest.xml;then
    mount -o bind system/oneplus6/empty /system/etc/permissions/android.hardware.fingerprint.xml
fi

if ! grep android.hardware.ir /vendor/manifest.xml;then
    mount -o bind system/oneplus6/empty /system/etc/permissions/android.hardware.consumerir.xml
fi
